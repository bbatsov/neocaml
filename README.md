...
- Dream eml template editing (`neocaml-eml-mode`) for `.eml.ml`, `.eml.html` and `.eml.re` files, with OCaml injected into the code regions and HTML into the template text (Emacs 30+)
...
- Easy installation of `ocaml` and `ocaml-interface` tree-sitter grammars via `M-x neocaml-install-grammars` (and the `tsx`/JSX grammar via `M-x neocaml-mlx-install-grammar`, the `eml` grammar via `M-x neocaml-eml-install-grammar`)
